...
namespace CSL1
{
    public partial class Form2 : Form
    {
        ...
        {            
            ...
            {
                isMouseDown = true;
                rectangle = new Rect(e.Location, e.Location, f1.brushSize, f1.colorLine, f1.colorFon); //Создаём новый объект класса Rect c начальными координатами в точке нажатия мыши
        ...
        private void Form2_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (isMouseDown)
            {
                ...
                if (rectangle.endPoint.X < Size.Width && rectangle.endPoint.Y < Size.Height
                && rectangle.startPoint.X > 0 && rectangle.startPoint.Y > 0)
                {
                    flagIzmen = true; //мы изменяли текущий файл
                    rectangle.Draw(g,AutoScrollPosition);
                    figures.Add(rectangle); //Добавление объекта в List
                }             
                 else rectangle.Hide(g);                  
